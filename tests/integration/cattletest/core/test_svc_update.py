...
def test_update_check_revision(client, super_client, context):
    ...
    for rev in revisions:
        if rev.id != rev1.id:
            assert rev.specs[svc.name]['capAdd'] == ['AUDIT_WRITE']
            rev2_id = rev.id
            break
...
def test_upgrade_global_service(new_context):
    ...
    service = client.update(service, launchConfig=launch_config)
    service = client.wait_success(service)
    assert service.state == 'active'
...
def test_rollback_to_revision(context, client, super_client):
    env = client.create_stack(name=random_str())
    env = client.wait_success(env)
    assert env.state == 'active'
    image_uuid = context.image_uuid
