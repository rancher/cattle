...
def test_agent_instance_create(admin_client, sim_context):
    ...
    ni2 = create_and_activate(admin_client, 'agentInstanceProvider',
                              networkId=network.id,
                              agentInstanceImageUuid=sim_context['imageUuid'])
    network_service = create_and_activate(admin_client, 'networkService',
    ...
    ni = admin_client.reload(ni)
    ni2 = admin_client.reload(ni2)
    ...
    agent_instance = admin_client.wait_success(maps[0].instance())
    agent_instance2 = admin_client.wait_success(maps2[0].instance())
...
def test_agent_instance_two_vnet_create(admin_client, sim_context,
                                        sim_context2):
    ...
    subnet = create_and_activate(admin_client, 'subnet',
                                 ...
                                 networkId=network.id)
    nsip = create_and_activate(admin_client, 'agentInstanceProvider',
                               networkId=network.id,
                               agentInstanceImageUuid=sim_context['imageUuid'])
    create_and_activate(admin_client, 'networkService',
    ...
    c2 = admin_client.create_container(imageUuid=sim_context2['imageUuid'],
