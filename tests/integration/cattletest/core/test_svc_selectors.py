...
def test_remove_unmanaged(client, context, super_client):
    env = _create_stack(client)
    ...
    svc = client.create_service(name=random_str(),
                                environmentId=env.id,
                                launchConfig=launch_config,
                                selectorContainer="foo2=bar2")
    svc = client.wait_success(svc)
    assert svc.selectorContainer == "foo2=bar2"
    ...
    maps = client.list_serviceExposeMap(serviceId=svc.id,
                                        state='active')
    assert maps[0].instanceId == c.id

    # update and remove the service
    # validate the instance is removed
    svc = super_client.update(svc, removeUnmanaged=True)
    svc = super_client.wait_success(svc)
    assert svc.removeUnmanaged is True

    svc = client.wait_success(svc.remove())
    wait_for(
        lambda: len(client.
                    list_serviceExposeMap(serviceId=svc.id,
                                          state='active')) == 0
    )
    wait_for(lambda: client.reload(c).state == 'removed',
             timeout=5)
