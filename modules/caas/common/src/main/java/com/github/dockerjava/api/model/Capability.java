 ...
 * 
 * @see <a href=
 *      "http://man7.org/linux/man-pages/man7/capabilities.7.html">http://man7.org/linux/man-pages/man7/capabilities.7.html</a>
 */
public enum Capability {
     ...
     * system UID of the process to match the UID of the file (e.g., chmod(2),
     * utime(2)), excluding those operations covered by the
     * {@link #DAC_OVERRIDE} and{@link #DAC_READ_SEARCH}.
    ...
    /**
     * <ul>
     * <li>Don't clear set-user-ID and set-group-ID permission bits when a file
     * is modified.
     * <li>Set the set-group-ID bit for a file whose GID does not match the file
     ...
     * <ul>
     * <li>grant or remove any capability in the caller's permitted capability
     * set to or from any other process. (This property of CAP_SETPCAP is not
    ...
    /**
     * <ul>
     * <li>Make arbitrary manipulations of process UIDs (setuid(2), setreuid(2),
     ...
     * <li>Perform IPC_SET and IPC_RMID operations on arbitrary System V IPC
     * objects.
    ...
    /**
     * <ul>
     * <li>Perform privileged syslog(2) operations. See syslog(2) for
     * information on which operations require privilege.
     * <li>View kernel addresses exposed via /proc and other interfaces when
     * /proc/sys/kernel/kptr_restrict has the value 1. (See the discussion of
    ...
    /**
     * <ul>
     * <li>Load and unload kernel modules (see init_module(2) and
     * delete_module(2))
    ...
    /**
     * <ul>
     * <li>Raise process nice value (nice(2), setpriority(2)) and change the
     * nice value for arbitrary processes.
     * <li>Set real-time scheduling policies for calling process, and set
     ...
     * (ioprio_set(2)).
     * <li>Apply migrate_pages(2) to arbitrary processes and allow processes to
     ...
     * <li>Override the /proc/sys/fs/pipe-size-max limit when setting the
    ...
    SYS_TTY_CONFIG,
    /**
     * Trigger something that will wake up the system (set CLOCK_REALTIME_ALARM
