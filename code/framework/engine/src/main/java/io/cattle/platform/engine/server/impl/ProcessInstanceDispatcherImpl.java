...
public class ProcessInstanceDispatcherImpl implements ProcessInstanceExecutor {
    ...
    public ProcessInstance execute(final long processId) {
        return lockManager.lock(new ProcessDispatchLock(processId), () -> {
            return processExecuteWithLock(processId);
        });
    }

    public ProcessInstance processExecuteWithLock(long processId) {
        EVENT.inc();
