...
public class AllocatorDaoImpl extends AbstractJooqDao implements AllocatorDao {
    ...
    public boolean isInstanceImageKind(long instanceId, String kind) {
        return create().select(STORAGE_POOL.fields()).from(STORAGE_POOL).join(IMAGE_STORAGE_POOL_MAP)
                .on(STORAGE_POOL.ID.eq(IMAGE_STORAGE_POOL_MAP.STORAGE_POOL_ID)).join(IMAGE).on(IMAGE.ID.eq(IMAGE_STORAGE_POOL_MAP.IMAGE_ID)).join(INSTANCE)
                .on(INSTANCE.IMAGE_ID.eq(IMAGE.ID))
                .where(INSTANCE.ID.eq(instanceId).and(IMAGE_STORAGE_POOL_MAP.REMOVED.isNull()).and(STORAGE_POOL.KIND.eq(kind))).fetch().size() > 0;
    }
    ...
    public List<? extends StoragePool> getAssociatedPools(Volume volume) {
        return create().select(STORAGE_POOL.fields()).from(STORAGE_POOL).join(VOLUME_STORAGE_POOL_MAP)
                .on(VOLUME_STORAGE_POOL_MAP.STORAGE_POOL_ID.eq(STORAGE_POOL.ID))
                .where(VOLUME_STORAGE_POOL_MAP.REMOVED.isNull().and(VOLUME_STORAGE_POOL_MAP.VOLUME_ID.eq(volume.getId()))).fetchInto(StoragePoolRecord.class);
    }

    @Override
    public List<? extends StoragePool> getAssociatedPools(Host host) {
        return create().select(STORAGE_POOL.fields()).from(STORAGE_POOL).join(STORAGE_POOL_HOST_MAP)
                .on(STORAGE_POOL_HOST_MAP.STORAGE_POOL_ID.eq(STORAGE_POOL.ID))
                .where(STORAGE_POOL_HOST_MAP.REMOVED.isNull().and(STORAGE_POOL_HOST_MAP.HOST_ID.eq(host.getId()))).fetchInto(StoragePoolRecord.class);
    }

    @Override
    public List<? extends Host> getHosts(Instance instance) {
        return create().select(HOST.fields()).from(HOST).join(INSTANCE_HOST_MAP).on(INSTANCE_HOST_MAP.HOST_ID.eq(HOST.ID))
                .where(INSTANCE_HOST_MAP.REMOVED.isNull().and(INSTANCE_HOST_MAP.INSTANCE_ID.eq(instance.getId()))).fetchInto(HostRecord.class);
    }

    @Override
    public List<? extends Host> getHosts(StoragePool pool) {
        return create().select(HOST.fields()).from(HOST).join(STORAGE_POOL_HOST_MAP).on(STORAGE_POOL_HOST_MAP.HOST_ID.eq(HOST.ID))
                .where(STORAGE_POOL_HOST_MAP.REMOVED.isNull().and(STORAGE_POOL_HOST_MAP.STORAGE_POOL_ID.eq(pool.getId()))).fetchInto(HostRecord.class);
    }
    ...
    protected void modifyCompute(long hostId, Instance instance, boolean add) {
        ...
        log.debug("Modifying computeFree on host [{}], {} {} {} = {}", host.getId(), computeFree, add ? "+" : "-", delta, newValue);
    ...
    public boolean recordCandidate(AllocationAttempt attempt, AllocationCandidate candidate) {
        ...
        if (existingHosts.size() == 0) {
            for (long hostId : newHosts) {
                ...
                objectManager.create(InstanceHostMap.class, INSTANCE_HOST_MAP.HOST_ID, hostId, INSTANCE_HOST_MAP.INSTANCE_ID, attempt.getInstance().getId());

                modifyCompute(hostId, attempt.getInstance(), false);
            }
        } else {
            if (!existingHosts.equals(newHosts)) {
                log.error("Can not move allocated instance [{}], currently {} new {}", attempt.getInstance().getId(), existingHosts, newHosts);
                throw new IllegalStateException("Can not move allocated instance [" + attempt.getInstance().getId() + "], currently " + existingHosts + " new "
                        + newHosts);
            }
        }
        ...
        if (isEmtpy(existingPools)) {
            for (Map.Entry<Long, Set<Long>> entry : newPools.entrySet()) {
                ...
                for (long poolId : entry.getValue()) {
                    ...
                    // Boolean stateCheck =
                    // AllocatorUtils.checkAllocateState(v.getId(),
                    // v.getAllocationState(), "Volume");
                    ...
                    if (inRightState) {
                        ...
                        objectManager.create(VolumeStoragePoolMap.class, VOLUME_STORAGE_POOL_MAP.VOLUME_ID, volumeId, VOLUME_STORAGE_POOL_MAP.STORAGE_POOL_ID,
                                poolId);
                    }
                }
            }
        ...
        for (Nic nic : attempt.getNics()) {
            ...
            int i = create().update(NIC).set(NIC.SUBNET_ID, subnetId).where(NIC.ID.eq(nic.getId())).execute();

            if (i != 1) {
                throw new IllegalStateException("Expected to update nic id [" + nic.getId() + "] with subnet [" + subnetId + "] but update [" + i + "] rows");
            }
        }
    ...
    public void releaseAllocation(Instance instance) {
        for (InstanceHostMap map : mapDao.findNonRemoved(InstanceHostMap.class, Instance.class, instance.getId())) {
            DataAccessor data = DataAccessor.fromDataFieldOf(map).withScope(AllocatorDaoImpl.class).withKey("deallocated");
    ...
    public List<Long> getHostsForSubnet(long subnetId, Long vnetId) {
        return create().select(HOST_VNET_MAP.HOST_ID).from(SUBNET_VNET_MAP).join(VNET).on(VNET.ID.eq(SUBNET_VNET_MAP.VNET_ID)).join(HOST_VNET_MAP)
                .on(HOST_VNET_MAP.VNET_ID.eq(VNET.ID))
                .where(SUBNET_VNET_MAP.SUBNET_ID.eq(subnetId).and(vnetCondition(vnetId)).and(HOST_VNET_MAP.STATE.eq(CommonStatesConstants.ACTIVE)))
    ...
    public List<Long> getPhysicalHostsForSubnet(long subnetId, Long vnetId) {
        return create()
                ...
                .where(SUBNET_VNET_MAP.SUBNET_ID.eq(subnetId).and(HOST_VNET_MAP.STATE.eq(CommonStatesConstants.ACTIVE)).and(vnetCondition(vnetId))
                        .and(HOST.PHYSICAL_HOST_ID.isNotNull())).fetchInto(Long.class);
    }
