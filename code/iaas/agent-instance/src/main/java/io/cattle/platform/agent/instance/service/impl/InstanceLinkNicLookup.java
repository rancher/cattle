...
public class InstanceLinkNicLookup extends AbstractJooqDao implements InstanceNicLookup {
    ...
    public List<Nic> getNics(Object obj) {
        ...
        if (obj instanceof InstanceLink) {
            ...
            nic = create().select(NIC.fields()).from(INSTANCE_LINK).join(NIC).on(NIC.INSTANCE_ID.eq(INSTANCE_LINK.INSTANCE_ID))
                    .where(NIC.DEVICE_NUMBER.eq(0).and(INSTANCE_LINK.ID.eq(link.getId()))).fetchOneInto(NicRecord.class);
        }
