    ...
    public List<? extends Nic> getNics(Object obj) {
        ...
        if (host.getPhysicalHostId() == null) {
            return create()
                    ...
                    .where(INSTANCE_HOST_MAP.HOST_ID.eq(((HostIpAddressMap) obj).getHostId()).and(NIC.REMOVED.isNull()).and(INSTANCE_HOST_MAP.REMOVED.isNull()))
            ...
            return create()
                    ...
                    .join(NIC)
                    .on(NIC.INSTANCE_ID.eq(INSTANCE_HOST_MAP.INSTANCE_ID))
                    .where(HOST.PHYSICAL_HOST_ID.eq(host.getPhysicalHostId()).and(HOST.REMOVED.isNull()).and(NIC.REMOVED.isNull())
                            .and(INSTANCE_HOST_MAP.REMOVED.isNull())).fetchInto(NicRecord.class);
        }
    }
