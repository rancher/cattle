...
public class IpPoolCreate extends AbstractDefaultProcessHandler {
    ...
    protected HandlerResult createSubnetPool(IpPool pool, ProcessState state) {
        ...
        if ( subnet == null ) {
            subnet = getObjectManager().create(Subnet.class,
                    SUBNET.ACCOUNT_ID, pool.getAccountId(),
                    SUBNET.IP_POOL_ID, pool.getId(),
                    SUBNET.KIND, SubnetConstants.KIND_IP_POOL_SUBNET,
                    SUBNET.GATEWAY, DataAccessor.fieldString(pool, IpPoolConstants.FIELD_GATEWAY),
                    SUBNET.END_ADDRESS, DataAccessor.fieldString(pool, IpPoolConstants.FIELD_END_ADDRESS),
                    SUBNET.START_ADDRESS, DataAccessor.fieldString(pool, IpPoolConstants.FIELD_START_ADDRESS),
                    SUBNET.CIDR_SIZE, DataAccessor.fieldString(pool, IpPoolConstants.FIELD_CIDR_SIZE),
                    SUBNET.NETWORK_ADDRESS, DataAccessor.fieldString(pool, IpPoolConstants.FIELD_NETWORK_ADDRESS));
        }
