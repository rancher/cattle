...
import io.cattle.platform.servicediscovery.deployment.impl.util.DeploymentUnitInstanceIdGeneratorImpl;
import io.cattle.platform.servicediscovery.healthaction.HealthCheckActionHandler;
import io.cattle.platform.servicediscovery.healthaction.NoopHealthCheckActionHandler;
import io.cattle.platform.servicediscovery.healthaction.RecreateHealthCheckActionHandler;
import io.cattle.platform.servicediscovery.healthaction.RecreateOnQuorumHealthCheckActionHandler;
 ...
 *
 */
public abstract class AbstractServiceDeploymentPlanner implements ServiceDeploymentPlanner {
    ...
    public AbstractServiceDeploymentPlanner(Service service,
    ...
    protected abstract List<DeploymentUnit> reconcileUnitsList(DeploymentUnitInstanceIdGenerator svcInstanceIdGenerator);

    protected abstract boolean needToReconcileScale();

    protected abstract void checkScale();

    @Override
    public String getStatus() {
        return String.format("Created: %d, Unhealthy: %d",
                healthyUnits.size(),
                unhealthyUnits.size());
    }
    ...
    protected void setHealthCheckAction(Service service, DeploymentManagerContext context) {
        Object healthCheckObj = ServiceUtil.getLaunchConfigObject(service,
    ...
    public List<DeploymentUnit> deploy() {
         ...
         */
        List<DeploymentUnit> units = this.reconcileUnitsList(getIdGenerator());
        sortByCreated(units);
