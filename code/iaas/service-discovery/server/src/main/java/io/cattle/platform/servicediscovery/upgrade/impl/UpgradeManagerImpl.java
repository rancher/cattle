            ...
            protected void markForCleanup(final long batchSize,
                ...
                while (it.hasNext() && i < batchSize) {
                    ...
                    for (Instance instance : instances.getValue()) {
                        activityService.instance(instance, "mark.upgrade", "Mark for upgrade");
    ...
    protected Service sleep(Service service, ServiceUpgradeStrategy strategy) {
        ...
        for (int i = 0;; i++) {
            ...
            if (sleepTime == 0) {
                ...
                activityService.run(service, "sleep", String.format("Sleeping for %d seconds", sleepTime/1000), new Runnable() {
                    @Override
                    public void run() {
                        try {
