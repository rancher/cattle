...
public class DeploymentUnitInstancePreCreate extends AbstractObjectProcessLogic implements ProcessPreListener, Priority {
    ...
    public HandlerResult handle(ProcessState state, ProcessInstance process) {
        ...
        if (!instance.getNativeContainer() && !state.getData().isEmpty()) {
            Long revisionId = instance.getRevisionId();
            if (revisionId == null) {
                InstanceRevision revision = instanceDao.createRevision(instance, state.getData(), true);
                revisionId = revision.getId();
            }

            objectManager.setFields(instance, InstanceConstants.FIELD_REVISION_ID, revisionId);
        }
        return null;
    }
