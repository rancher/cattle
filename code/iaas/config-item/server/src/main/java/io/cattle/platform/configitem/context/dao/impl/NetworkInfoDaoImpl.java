...
public class NetworkInfoDaoImpl extends AbstractJooqDao implements NetworkInfoDao {
    ...
    public List<NetworkClientData> networkClients(Instance instance) {
        ...
        return create()
                .select(clientNic.DEVICE_NUMBER.as("client_device_number"), clientNic.MAC_ADDRESS.as("mac_address"),
                        instanceNic.DEVICE_NUMBER.as("instance_device_number"), INSTANCE.DOMAIN.as("instance_domain"), INSTANCE.HOSTNAME.as("hostname"),
                        INSTANCE.ID.as("instance_id"), INSTANCE.UUID.as("instance_uuid"), IP_ADDRESS.ADDRESS.as("ip_address"),
                        NETWORK.DOMAIN.as("network_domain"), SUBNET.GATEWAY.as("gateway"))
                .from(instanceNic)
                .join(NETWORK)
                ...
                .join(IP_ADDRESS)
                .on(IP_ADDRESS.ID.eq(IP_ADDRESS_NIC_MAP.IP_ADDRESS_ID).and(IP_ADDRESS.ROLE.eq(IpAddressConstants.ROLE_PRIMARY)))
                .join(SUBNET)
                .on(IP_ADDRESS.SUBNET_ID.eq(SUBNET.ID))
                .where(instanceNic.INSTANCE_ID.eq(instance.getId()).and(IP_ADDRESS.REMOVED.isNull()).and(IP_ADDRESS_NIC_MAP.REMOVED.isNull())
                        .and(INSTANCE.REMOVED.isNull()).and(clientNic.REMOVED.isNull()).and(instanceNic.REMOVED.isNull())).fetchInto(NetworkClientData.class);
    }
    ...
    public List<NetworkClientData> vnetClients(Instance instance) {
        ...
        return create()
                .select(clientNic.DEVICE_NUMBER.as("client_device_number"), clientNic.MAC_ADDRESS.as("mac_address"),
                        instanceNic.DEVICE_NUMBER.as("instance_device_number"), INSTANCE.DOMAIN.as("instance_domain"), INSTANCE.HOSTNAME.as("hostname"),
                        INSTANCE.ID.as("instance_id"), INSTANCE.UUID.as("instance_uuid"), IP_ADDRESS.ADDRESS.as("ip_address"), SUBNET.GATEWAY.as("gateway"))
                .from(instanceNic)
                .join(VNET)
    ...
    public List<? extends NetworkService> networkServices(Instance instance) {
        return create().select(NETWORK_SERVICE.fields()).from(NETWORK_SERVICE).join(NIC).on(NIC.NETWORK_ID.eq(NETWORK_SERVICE.NETWORK_ID))
                .join(NETWORK_SERVICE_PROVIDER).on(NETWORK_SERVICE.NETWORK_SERVICE_PROVIDER_ID.eq(NETWORK_SERVICE_PROVIDER.ID))
                .where(NIC.INSTANCE_ID.eq(instance.getId()).and(NETWORK_SERVICE_PROVIDER.KIND.eq(NetworkServiceProviderConstants.KIND_AGENT_INSTANCE)))
    ...
    public List<? extends NetworkService> networkServices(Agent agent) {
        return create().select(NETWORK_SERVICE.fields()).from(NETWORK_SERVICE).join(NIC).on(NIC.NETWORK_ID.eq(NETWORK_SERVICE.NETWORK_ID))
                .join(NETWORK_SERVICE_PROVIDER).on(NETWORK_SERVICE.NETWORK_SERVICE_PROVIDER_ID.eq(NETWORK_SERVICE_PROVIDER.ID)).join(INSTANCE_HOST_MAP)
                .on(INSTANCE_HOST_MAP.INSTANCE_ID.eq(NIC.INSTANCE_ID)).join(HOST).on(HOST.ID.eq(INSTANCE_HOST_MAP.HOST_ID))
                .where(HOST.AGENT_ID.eq(agent.getId()).and(NETWORK_SERVICE_PROVIDER.KIND.eq(NetworkServiceProviderConstants.KIND_AGENT_INSTANCE)))
                .fetchInto(NetworkServiceRecord.class);
    }
    ...
    public Map<Long, Network> networks(Instance instance) {
        ...
        List<NetworkRecord> records = create().select(NETWORK.fields()).from(NETWORK).join(NIC).on(NETWORK.ID.eq(NIC.NETWORK_ID))
                .where(NIC.INSTANCE_ID.eq(instance.getId())).fetchInto(NetworkRecord.class);
    ...
    public List<InstanceLinkData> getLinks(Instance instance) {
        ...
        return create()
                ...
                .where(NIC.INSTANCE_ID.eq(instance.getId()).and(NIC.VNET_ID.isNotNull()).and(NIC.REMOVED.isNull())
                        .and(ipAddress.ROLE.eq(IpAddressConstants.ROLE_PRIMARY)).and(ipAddress.REMOVED.isNull()).and(IP_ADDRESS_NIC_MAP.REMOVED.isNull())
                        .and(clientNic.REMOVED.isNull()).and(targetNic.REMOVED.isNull()).and(instanceLink.REMOVED.isNull())).fetch().map(mapper);
    }
    ...
    public List<ClientIpsecTunnelInfo> getIpsecTunnelClient(Instance inputAgentInstance) {
        ...
        MultiRecordMapper<ClientIpsecTunnelInfo> mapper = new MultiRecordMapper<ClientIpsecTunnelInfo>() {
            ...
            protected ClientIpsecTunnelInfo map(List<Object> input) {
                ...
                if (instance.getAgentId() != null) {
                    Map<?, ?> map = DataAccessor.fromDataFieldOf(instance).withScopeKey("ipsec").withKey(Long.toString(host.getId()))
                            .withDefault(new HashMap<String, Object>()).as(jsonMapper, Map.class);
        ...
        List<ClientIpsecTunnelInfo> tempResult = create()
                ...
                .where(NIC.NETWORK_ID.eq(primaryNic.getNetworkId()).and(clientIpAddress.ROLE.eq(IpAddressConstants.ROLE_PRIMARY)).and(NIC.REMOVED.isNull())
                        .and(HOST_IP_ADDRESS_MAP.REMOVED.isNull()).and(IP_ADDRESS_NIC_MAP.REMOVED.isNull()).and(clientIpAddress.REMOVED.isNull())
                        .and(ipAddress.REMOVED.isNull()).and(host.REMOVED.isNull()).and(subnet.REMOVED.isNull()).and(instance.REMOVED.isNull())).fetch()
                .map(mapper);
    ...
    public List<HostPortForwardData> getHostPorts(Agent agent) {
        ...
        return create()
                ...
                .where(HOST.AGENT_ID.eq(agent.getId()).and(INSTANCE_HOST_MAP.REMOVED.isNull()).and(port.REMOVED.isNull()).and(port.PUBLIC_PORT.isNotNull())
                        .and(port.STATE.in(CommonStatesConstants.ACTIVATING, CommonStatesConstants.ACTIVE, CommonStatesConstants.UPDATING_ACTIVE))
                        .and(HOST.REMOVED.isNull())).orderBy(port.CREATED.asc()).fetch().map(mapper);
    }
    ...
    public List<IpAssociationData> getHostIps(Agent agent) {
        ...
        return create()
                ...
                .where(HOST.AGENT_ID.eq(agent.getId()).and(IP_ADDRESS_NIC_MAP.REMOVED.isNull()).and(INSTANCE_HOST_MAP.REMOVED.isNull())
                        .and(INSTANCE_HOST_MAP.STATE.in(CommonStatesConstants.ACTIVATING, CommonStatesConstants.ACTIVE)).and(ipAddress.REMOVED.isNull())
                        .and(IP_ASSOCIATION.REMOVED.isNull()).and(IP_ASSOCIATION.STATE.in(CommonStatesConstants.ACTIVATING, CommonStatesConstants.ACTIVE))
                        .and(ipAddress.ADDRESS.isNotNull()).and(targetIpAddress.ADDRESS.isNotNull()).and(NIC.REMOVED.isNull())).fetch().map(mapper);
    }
    ...
    public List<HostRouteData> getHostRoutes(Agent agent) {
        ...
        return create()
                ...
                .on(networkService.NETWORK_ID.eq(NIC.NETWORK_ID).and(networkService.KIND.eq(NetworkServiceConstants.KIND_HOST_NAT_GATEWAY)))
                .where(HOST.AGENT_ID.eq(agent.getId()).and(IP_ADDRESS.ROLE.eq(IpAddressConstants.ROLE_PRIMARY)).and(HOST.REMOVED.isNull())
                        .and(INSTANCE_HOST_MAP.REMOVED.isNull()).and(NIC.REMOVED.isNull()).and(instance.REMOVED.isNull())
                        .and(IP_ADDRESS_NIC_MAP.REMOVED.isNull()).and(subnet.REMOVED.isNull())).fetch().map(mapper);
    }
    ...
    public Map<Nic, Subnet> getNicsAndSubnet(Instance instance) {
        ...
        create().select(mapper.fields()).from(nic).leftOuterJoin(subnet).on(nic.SUBNET_ID.eq(subnet.ID)).where(nic.INSTANCE_ID.eq(instance.getId())).fetch()
                .map(mapper);
