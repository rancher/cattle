    ...
    protected void handleApplied(Request req) {
        ...
        if (version.isLatest()) {
            ...
            if (item.getSourceRevision().equals(version.getSourceRevision())) {
                ...
                log.info("Ignoring item [{}] to version [{}] for [{}] because source revision [{}] does not match expect [{}]", req.getItemName(),
                        req.getAppliedVersion(), req.getClient(), version.getSourceRevision(), item.getSourceRevision());
            }
        }
    }
