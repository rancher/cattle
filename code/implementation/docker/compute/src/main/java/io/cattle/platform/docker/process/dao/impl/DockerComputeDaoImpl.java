...
public class DockerComputeDaoImpl extends AbstractJooqDao implements DockerComputeDao {
    ...
    public IpAddress getDockerIp(String ip, Instance instance) {
        ...
        List<IpAddressRecord> ips = create().select(IP_ADDRESS.fields()).from(IP_ADDRESS).join(IP_ADDRESS_NIC_MAP)
                .on(IP_ADDRESS_NIC_MAP.IP_ADDRESS_ID.eq(IP_ADDRESS.ID)).join(NIC).on(IP_ADDRESS_NIC_MAP.NIC_ID.eq(NIC.ID))
                .where(NIC.INSTANCE_ID.eq(instance.getId()).and(IP_ADDRESS.KIND.eq(DockerIpAddressConstants.KIND_DOCKER).or(IP_ADDRESS.ADDRESS.eq(ip))))
    ...
    public Volume getDockerVolumeInPool(String volumeUri, StoragePool storagePool) {
        ...
        List<VolumeRecord> volumes = create().select(VolumeTable.VOLUME.fields()).from(VOLUME_STORAGE_POOL_MAP).join(VOLUME)
                .on(VOLUME_STORAGE_POOL_MAP.VOLUME_ID.eq(VOLUME.ID)).where(VOLUME_STORAGE_POOL_MAP.STORAGE_POOL_ID.eq(storagePool.getId()))
                .and(VOLUME.URI.eq(volumeUri)).and(VOLUME_STORAGE_POOL_MAP.REMOVED.isNull()).and(VOLUME.REMOVED.isNull()).fetchInto(VolumeRecord.class);
        ...
        if (volumes.isEmpty())
            ...
            throw new IllegalStateException(String.format("More than one volume exists for volume URI [%s] and storage pool [%s].", volumeUri,
                    storagePool.getId()));
    }

    @Override
    public Volume createDockerVolumeInPool(Long accountId, String volumeUri, StoragePool storagePool, boolean isHostPath) {
        Volume volume = objectManager.create(Volume.class, VOLUME.ACCOUNT_ID, accountId, VOLUME.ATTACHED_STATE, CommonStatesConstants.INACTIVE,
                VOLUME.DEVICE_NUMBER, -1, VOLUME.ALLOCATION_STATE, CommonStatesConstants.ACTIVE, VOLUME.URI, volumeUri);

        DataAccessor.fields(volume).withKey(FIELD_DOCKER_IS_HOST_PATH).set(isHostPath);
        objectManager.persist(volume);

        objectManager.create(VolumeStoragePoolMap.class, VOLUME_STORAGE_POOL_MAP.VOLUME_ID, volume.getId(), VOLUME_STORAGE_POOL_MAP.STORAGE_POOL_ID,
                storagePool.getId());
