...
public class DockerTransformerTest {
    ...
    public void testTransform() {
        ...
        assertEquals("docker:" + configInput.get("Image"), DataAccessor.fields(instance).withKey(FIELD_IMAGE_UUID).get());
        ...
        Map<String, String> instanceEnvs = (Map<String, String>)DataAccessor.fields(instance).withKey(FIELD_ENVIRONMENT).get();
        ...
        List<String> instanceCmd = (List<String>)DataAccessor.fields(instance).withKey(FIELD_COMMAND).get();
        for (int i = 0; i < inputCmd.length; i++) {
            assertEquals(inputCmd[i], instanceCmd.get(i));
        }
        ...
        assertEquals(hostConfigInput.get("PublishAllPorts"), DataAccessor.fields(instance).withKey(FIELD_PUBLISH_ALL_PORTS).get());

        Map[] inputLxcConfs = (Map[])hostConfigInput.get("LxcConf");
        Map<String, String> instanceLxc = (Map<String, String>)DataAccessor.fields(instance).withKey(FIELD_LXC_CONF).get();
        ...
        io.cattle.platform.core.addon.RestartPolicy instanceRP = (io.cattle.platform.core.addon.RestartPolicy)DataAccessor.fields(instance)
                .withKey(FIELD_RESTART_POLICY).get();
    ...
    public void transformVolumes() {
        ...
        Map<String, Boolean> volumesRW = new HashMap<String, Boolean>();
        inspect.put("VolumesRW", volumesRW);
        Map<String, String> volumes = new HashMap<String, String>();
        inspect.put("Volumes", volumes);

        volumes.put("/foo", "/mnt/sda1/var/lib/docker/vfs/dir/123");
        volumes.put("/bar", "/mnt/sda1/tmp/bar");
        volumes.put("/baz", "/mnt/sda1/tmp/baz");

        volumesRW.put("/foo", Boolean.TRUE);
        volumesRW.put("/bar", Boolean.TRUE);
        volumesRW.put("/baz", Boolean.FALSE);

        binds.add("/tmp/bar:/bar:rw");
        binds.add("/tmp/baz:/baz:ro");

        DockerTransformerImpl transformer = transformer();
        List<DockerInspectTransformVolume> vols = transformer.transformVolumes(inspect);
        DockerInspectTransformVolume foo = null;
        DockerInspectTransformVolume bar = null;
        DockerInspectTransformVolume baz = null;
        for (DockerInspectTransformVolume v : vols) {
            if (v.getContainerPath().contains("foo"))
                foo = v;
            else if (v.getContainerPath().contains("bar"))
                bar = v;
            else if (v.getContainerPath().contains("baz"))
                baz = v;
        }

        assertEquals("rw", foo.getAccessMode());
        assertEquals("/foo", foo.getContainerPath());
        assertEquals("/mnt/sda1/var/lib/docker/vfs/dir/123", foo.getHostPath());

        assertEquals("rw", bar.getAccessMode());
        assertEquals("/bar", bar.getContainerPath());
        assertEquals("/mnt/sda1/tmp/bar", bar.getHostPath());

        assertEquals("ro", baz.getAccessMode());
        assertEquals("/baz", baz.getContainerPath());
        assertEquals("/mnt/sda1/tmp/baz", baz.getHostPath());
                ...
                if (configAccessor.getKey().equals("Links")) {
                    ...
                    for (LxcConf lxcConf : actualLxcConfs) {
                        ...
                        for (Map expectedLxc : expectedLxcConf) {
                            if (lxcConf.getKey().equals(expectedLxc.get("Key")) && lxcConf.getValue().equals(expectedLxc.get("Value"))) {
                                found = true;
                                break;
                            }
                        }
                        ...
                        if (expectedValue != null) {
                            ...
                            for (Binding b : binding.getValue()) {
                                if (!b.getHostIp().equals(expectedValue.get(index).get("HostIp"))
                                        || !b.getHostPort().equals(Integer.valueOf((String)expectedValue.get(index).get("HostPort")))) {
                                    fail();
                                }
                                index++;
                            }
                    ...
                    for (Device d1 : actualDevices) {
                        ...
                        String perms = StringUtils.isEmpty((String)d2.get("CgroupPermissions")) ? "" : (String)d2.get("CgroupPermissions");
    ...
    @SuppressWarnings({ "unchecked", "rawtypes" })
    void putConfigField(Map<String, Method> configFields, Map<String, Object> config, Class clazz, String field, Object value, String methodName)
    ...
    Map<String, Object> inspect(Map<String, Method> configFields, Map<String, Method> hostConfigFields) {
        ...
        try {
            ...
            putConfigField(configFields, config, configClazz, "Env", new String[] { "env1=true", "env2=next" }, "getEnv");
            putConfigField(configFields, config, configClazz, "Cmd", new String[] { "sleep", "1" }, "getCmd");
            putConfigField(configFields, config, configClazz, "Entrypoint", new String[] { "entrypoint", "2" }, "getEntrypoint");
            ...
            putConfigField(hostConfigFields, hostConfig, hostConfigClazz, "Binds", new String[] { "/tmp:/bar", "/var:/baz:ro" }, "getBinds");
            putConfigField(hostConfigFields, hostConfig, hostConfigClazz, "Links", new String[] { "a:b", "c:d" }, "getLinks");

            putConfigField(hostConfigFields, hostConfig, hostConfigClazz, "PublishAllPorts", Boolean.TRUE, "isPublishAllPorts");
            ...
            putConfigField(hostConfigFields, hostConfig, hostConfigClazz, "PortBindings", portBindings, "getPortBindings");
            putConfigField(hostConfigFields, hostConfig, hostConfigClazz, "Dns", new String[] { "1.1.1.1" }, "getDns");
            putConfigField(hostConfigFields, hostConfig, hostConfigClazz, "DnsSearch", new String[] { "a search domain" }, "getDnsSearch");
            putConfigField(hostConfigFields, hostConfig, hostConfigClazz, "ExtraHosts", new String[] { "host1:1.2.3.4" }, "getExtraHosts");
            putConfigField(hostConfigFields, hostConfig, hostConfigClazz, "VolumesFrom", new String[] { "containerA:rw", "containerB:ro" }, "getVolumesFrom");
            putConfigField(hostConfigFields, hostConfig, hostConfigClazz, "CapAdd", new String[] { "NET_ADMIN", "MKNOD" }, "getCapAdd");
            putConfigField(hostConfigFields, hostConfig, hostConfigClazz, "CapDrop", new String[] { "NET_ADMIN", "MKNOD" }, "getCapDrop");
            putConfigField(hostConfigFields, hostConfig, hostConfigClazz, "RestartPolicy", new HashMap<String, Object>() {
                {
                    put("Name", "on-failure");
