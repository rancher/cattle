...
class LibvirtStorage(KindBasedMixin, BaseStoragePool):
    ...
    def _is_image_active(self, image, storage_pool):
        return get_pool_driver(storage_pool).is_image_active(image,
                                                             storage_pool)

    def _do_image_activate(self, image, storage_pool, progress):
        return get_pool_driver(storage_pool).image_activate(image,
                                                            storage_pool,
                                                            progress)
    ...
    def _get_image_storage_pool_map_data(image_storage_pool_map):
        ...
        image_obj = get_pool_driver(storage_pool).get_image(image,
                                                            storage_pool)
    ...
    def _get_volume_storage_pool_map_data(volume_storage_pool_map):
        ...
        volume_obj = get_pool_driver(storage_pool).get_volume(volume,
                                                              storage_pool)
    ...
    def _is_volume_active(self, volume, storage_pool):
        return get_pool_driver(storage_pool).is_volume_active(volume,
                                                              storage_pool)

    def _do_volume_activate(self, volume, storage_pool, progress):
        return get_pool_driver(storage_pool).volume_activate(volume,
                                                             storage_pool,
                                                             progress)

    def _is_volume_inactive(self, volume, storage_pool):
        return get_pool_driver(storage_pool).is_volume_inactive(volume,
                                                                storage_pool)

    def _is_volume_removed(self, volume, storage_pool):
        return get_pool_driver(storage_pool).is_volume_removed(volume,
                                                               storage_pool)

    def _do_volume_remove(self, volume, storage_pool, progress):
        return get_pool_driver(storage_pool).volume_remove(volume,
                                                           storage_pool,
                                                           progress)
