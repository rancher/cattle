...
<#if dnsEntries?? >
    ...
    "${dnsEntry.sourceIpAddress.address}": {
        ...
        "${dnsName?lower_case}.": [<#list dnsEntry.resolve[dnsName] as address>"${address}"<#if address_has_next>, </#if></#list>],
        "${dnsName?lower_case}.rancher.internal.": [<#list dnsEntry.resolve[dnsName] as address>"${address}"<#if address_has_next>, </#if></#list>]<#if dnsName_has_next>,</#if>
                    </#if>
                </#list>
    },
        </#if>
